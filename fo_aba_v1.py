...
def detectar_abas(arquivo_bytes):
    """
    Detecta qual aba é a de Descricao e qual é o Plano Semanal
    inspecionando o CONTEUDO das células, não o nome da aba.

    Aba de Descricao:   contém 'CONTEXTUALIZAÇÃO' em alguma célula das 5 primeiras linhas
    Aba de Plano:       contém a coluna 'Aula' no cabeçalho (primeiras 5 linhas)

    Retorna: (nome_aba_descricao, nome_aba_plano)
    Lança ValueError se alguma aba não for encontrada.
    """
    ...
    for nome in wb.sheetnames:
        ...
        for row in ws.iter_rows(max_row=5, values_only=True):
            for cell in row:
                if cell is not None:
                    valores.add(str(cell).strip().upper())

        # Assinatura da aba de Descricao: tem 'CONTEXTUALIZAÇÃO'
        if aba_descricao is None and any(
            "CONTEXTUALIZA" in v for v in valores
        ):
            aba_descricao = nome

        # Assinatura da aba de Plano: tem coluna 'AULA'
        if aba_plano is None and "AULA" in valores:
            aba_plano = nome

    wb.close()

    erros = []
    if aba_descricao is None:
        erros.append(
            "Nenhuma aba encontrada com 'CONTEXTUALIZAÇÃO'. "
            "Verifique se o Excel possui a aba de descrição corretamente preenchida."
        )
    if aba_plano is None:
        erros.append(
            "Nenhuma aba encontrada com coluna 'Aula'. "
            "Verifique se o Excel possui a aba de plano semanal corretamente preenchida."
        )
    if erros:
        raise ValueError("\n".join(erros))

    return aba_descricao, aba_plano
...
def extrair_descricao_excel(arquivo_bytes):
    ...
    try:
        ...
        aba_descricao, _ = detectar_abas(arquivo_bytes)
...
def ler_plano_excel(arquivo_bytes):
    try:
        _, aba_plano = detectar_abas(arquivo_bytes)
        return pd.read_excel(
            io.BytesIO(arquivo_bytes),
            sheet_name=aba_plano,
            skiprows=2
        )
    except Exception as e:
